...
# Disable pynhd's broken aiosqlite cache (Py3.11+). Set before pynhd import.
...
# Performance Timer
# Uses st.status() when available (collapsible, native, cleaner); falls back
# to plain stdout + optional status_box for non-Streamlit callers.
@contextmanager
def task_timer(label: str, status_box=None):
    ...
    use_status = hasattr(st, "status") and status_box is None
    status_ctx = st.status(f"{label}...", expanded=False) if use_status else None

    if status_box and not use_status:
    ...
    try:
        ...
        if status_ctx is not None:
            status_ctx.update(label=f"{label} — failed", state="error", expanded=True)
        raise
    finally:
        ...
        print(f"{msg_end}\n")
        if status_ctx is not None:
            status_ctx.update(label=f"{label} ({duration:.1f}s)", state="complete", expanded=False)
        elif status_box:
...
# Map & UI Helpers — validation delegates to ui.validation (shared with CLI)
from ui.validation import (
    path_exists,
    validate_dem_file as _validate_dem_file_impl,
    validate_river_file as _validate_river_file_impl,
)

validate_dem_file = _validate_dem_file_impl
validate_river_file = _validate_river_file_impl

# Map + coverage helpers — also extracted to ui/map.py
from ui.map import (
    fetch_dem_coverage_footprints as _fetch_dem_coverage_footprints_impl,
    build_aoi_map as _build_aoi_map_impl,
    extract_aoi_from_map as _extract_aoi_from_map_impl,
    save_aoi_geojson as _save_aoi_geojson_impl,
)


def fetch_dem_coverage_footprints(bounds, radius_km: float = 100, aoi_geom=None):
    return _fetch_dem_coverage_footprints_impl(bounds, radius_km=radius_km, aoi_geom=aoi_geom, sink=st)


build_aoi_map = _build_aoi_map_impl
extract_aoi_from_map = _extract_aoi_from_map_impl
save_aoi_geojson = _save_aoi_geojson_impl
...
if st.session_state.get("run_requested"):
    ...
    if st.session_state["qa_approved"] and not st.session_state["rem_done"]:
        ...
        # Write reproducibility manifest next to outputs.
        try:
            manifest_payload = {
                "aoi_bbox": None,
                "config": {
                    "run_mode": run_mode,
                    "resolution": dem_res_input,
                    "river_name": selected_river_name,
                    "spacing": int(spacing),
                    "engine": selected_engine,
                    "k_neighbors": int(k_neighbors),
                    "max_value_comp": max_m_comp,
                    "max_value_vis": max_m_vis,
                    "ramps": selected_ramps,
                    "bg_type": bg_type,
                    "bg_alpha": float(bg_alpha),
                    "rem_alpha": float(rem_alpha),
                    "use_discrete_colors": bool(use_discrete_colors),
                    "cpu_util_percent": int(cpu_util_percent),
                    "data_source": data_source,
                },
                "outputs": {
                    "dem": dem_file,
                    "rem": rem_tif,
                    "hillshade": st.session_state.get("hillshade_output"),
                    "river": st.session_state.get("reprojected_river_path"),
                    "pngs": pngs,
                    "stats": stats_txt_path,
                },
                "crs": {
                    "dem_original": st.session_state.get("dem_original_crs"),
                    "dem_output": st.session_state.get("dem_output_crs"),
                    "river_original": st.session_state.get("river_original_crs"),
                    "river_output": st.session_state.get("river_output_crs"),
                },
                "cache_stats": rem_cache.cache_stats(),
            }
            aoi_path_for_manifest = st.session_state.get("aoi_geojson_path")
            if aoi_path_for_manifest and os.path.exists(aoi_path_for_manifest):
                try:
                    _g = gpd.read_file(aoi_path_for_manifest)
                    manifest_payload["aoi_bbox"] = [float(x) for x in _g.total_bounds]
                except Exception:
                    pass
            manifest_dir = st.session_state.get("project_folder_expanded", project_folder)
            rem_cache.write_manifest(manifest_dir, manifest_payload)
        except Exception as _mfe:
            print(f"Manifest write failed (non-fatal): {_mfe}")
