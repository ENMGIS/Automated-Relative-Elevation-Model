...
# pynhd's aiohttp_client_cache/aiosqlite backend is broken on Python 3.11+
# ('Connection' object has no attribute '_stop_running'). Disable it — we
# have our own in-memory cache for V2 bbox results anyway.
...
# In-memory cache for NHDPlus V2 flowline queries.
# Key: rounded bbox tuple. Value: GeoDataFrame in EPSG:4326.
# Avoids re-fetching the same bbox when scan_nhd_rivers is followed by
# choose_and_save_nhd_river in the same process.
_NHD_V2_BBOX_CACHE: dict = {}
_NHD_V2_CACHE_MAX = 8

def _nhd_cache_key(bounds) -> tuple:
    # Round to ~1m to collapse floating-point noise between callers.
    return tuple(round(float(v), 5) for v in bounds)

def _nhd_cache_get(bounds):
    return _NHD_V2_BBOX_CACHE.get(_nhd_cache_key(bounds))

def _nhd_cache_put(bounds, gdf) -> None:
    if len(_NHD_V2_BBOX_CACHE) >= _NHD_V2_CACHE_MAX:
        _NHD_V2_BBOX_CACHE.pop(next(iter(_NHD_V2_BBOX_CACHE)))
    _NHD_V2_BBOX_CACHE[_nhd_cache_key(bounds)] = gdf
...
def _download_file_worker(url, dest_path, tile_index=0, total_tiles=0):
    ...
    # Cross-run persistent cache: skip network if we've already downloaded this URL.
    if rem_cache.link_from_cache(url, dest_path):
        if validate_raster_integrity(dest_path):
            logger.info(f"  [{tile_index}/{total_tiles}] {filename} (from cache)")
            return dest_path
        # Cached copy is corrupt — fall through to re-download.
        try:
            os.remove(dest_path)
        except OSError:
            pass
    ...
    for attempt in range(1, cfg.DOWNLOAD_MAX_RETRIES + 1):
        try:
            ...
            # Validate
            if validate_raster_integrity(dest_path):
                rem_cache.store_in_cache(url, dest_path)
                return dest_path
            else:
...
def _download_via_source_api(aoi_gdf, output_folder, resolution, n_jobs=DEFAULT_JOBS, alaska_5m_strategy=None):
    ...
    # Add S1M tiles only if project tiles don't already fully cover the AOI.
    # Previously S1M were always added as a backup, which doubled download
    # volume whenever project coverage was already complete.
    if s1m_optimized:
        project_geoms = [t.get("geometry") for t in project_optimized if t.get("geometry") is not None]
        skip_s1m = False
        if project_geoms:
            try:
                project_union = unary_union(project_geoms)
                aoi_area = aoi_geom.area
                if aoi_area > 0:
                    covered_area = aoi_geom.intersection(project_union).area
                    coverage = covered_area / aoi_area
                    logger.info(f"Project tile AOI coverage: {coverage*100:.1f}%")
                    if coverage >= 0.99:
                        skip_s1m = True
            except Exception as e:
                logger.warning(f"Coverage check failed ({e}); falling back to always-include S1M")

        if skip_s1m:
            logger.info(f"Skipping {len(s1m_optimized)} S1M backup tiles (project tiles cover AOI).")
        else:
    ...
    for attempt in range(1, 4):
        try:
            ...
            if da.size > 0:
                ...
                if validate_raster_integrity(tile_path): return tile_path
        except Exception: time.sleep(5 * attempt)
    return None 
        ...
        try:
            ...
            if res is None:
                ...
                if res is not None and not res.empty:
                    ...
                    _nhd_cache_put(bounds, res)
            else:
                logger.info("Running Fast Scan (NHDPlus V2, cached)...")

            if res is not None and not res.empty:
                try: clipped = gpd.clip(res, gdf)
                except Exception: clipped = res.cx[gdf.total_bounds[0]:gdf.total_bounds[2], gdf.total_bounds[1]:gdf.total_bounds[3]]
    ...
    if is_conus:
        ...
        for g in geojson_files:
            try:
                ...
                fl = _nhd_cache_get(bounds)
                if fl is None:
                    fl = wd.bybox(bounds)
                    if fl is not None and not fl.empty:
                        fl = fl.to_crs("EPSG:4326")
                        _nhd_cache_put(bounds, fl)
                else:
                    logger.info("Using cached NHDPlus V2 flowlines for this AOI.")

                if fl is not None and not fl.empty:
                    try: clipped = gpd.clip(fl, aoi)
                    except Exception: clipped = fl.cx[aoi.total_bounds[0]:aoi.total_bounds[2], aoi.total_bounds[1]:aoi.total_bounds[3]]
